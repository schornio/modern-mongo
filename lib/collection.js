...
class Collection {
  ...
  async exists () {
    ...
    return Collection.exists(this.collectionName);
  ...
  async indexExists (...args) {
    ...
    const collection = await this.getNativeCollection();
    return await collection.indexExists(...args);
...
Collection.exists = async (collectionName) => {

    const { db } = await connect();
    const collections = await db.collections();
    const collectionNames = collections.map((collection) => collection.collectionName);

    for (const foundCollectionName of collectionNames) {

      if (collectionName === foundCollectionName) {
